...
import { DropdownMenuTrigger } from '#app/components/ui/dropdown-menu.tsx'
  ...
  React.useImperativeHandle(ref, () => ({
    onKeyDown: ({ event }: { event: React.KeyboardEvent }) => {
      if (event.key === 'ArrowDown') {
        ...
        if (commands?.length ?? 0 - 1 < newCommandIndex) {
          newCommandIndex = 0
      ...
      if (event.key === 'ArrowUp') {
        ...
        if (newCommandIndex < 0) {
          newGroupIndex = selectedGroupIndex - 1
          newCommandIndex =
            props.items[newGroupIndex]?.commands?.length ?? 0 - 1
        }
